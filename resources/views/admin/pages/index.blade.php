...
@section('title') Информационные страницы @endsection
@section('header')
    <!-- DataTables -->
...
@component('admin.layouts.components.breadcrumb')
    @slot('title') Информационные страницы @endslot
    @slot('active') Информационные страницы @endslot
{{--    @slot('a_2')  @endslot--}}
@endcomponent
        ...
        <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
            ...
            @foreach($pages as $page)
                <tr>
                    ...
                    <td>{{ $page->name }}</td>
                    <td>{{ mb_strimwidth(strip_tags($page->content), 0, 100, '...') }}</td>
                    <td>
                        <a href="#" class="mr-3 text-primary"><i class="ri-eye-line font-size-18"></i></a>
                        <a href="{{ route('admin.pages.info.edit', ['id' => $page->id]) }}" class="mr-3 text-warning">
                            <i class="ri-pencil-fill font-size-18"></i>
                        </a>

                        <a href="{{ route('admin.pages.info.destroy', ['id' => $page->id]) }}" class="text-danger"
                        ...
                        <form id="form_{{ $page->id }}" action="{{ route('admin.pages.info.destroy', ['id' => $page->id]) }}" method="POST" style="display: none;">
                            @csrf
                        </form>
                    </td>
                </tr>
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="myModalLabel">Новая страница</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <form id="save_form" action="{{ route('admin.pages.info.store') }}" method="post">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            <div>
                                <textarea id="elm1" name="content" rows="5" class="form-control"></textarea>
                            </div>
                        </div>
                    </form>
                </div>
